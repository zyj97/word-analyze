...
public class ChineseNumberUtils {
    ...
    private static Map<Integer, String> cdict = null;
    private static Map<Character, String> gdict = null;
     ...
     */
    private static List<String> csplit(String cdata) {
     ...
     */
    private static String cschange(String cki) {
        ...
        for (int i = 0; i < lenki; i++) {
            if (cki.charAt(i) == '0') {
                if (i < lenki - 1) {
                    if (cki.charAt(i + 1) != '0') {
                        chk = chk + getGdict().get(cki.charAt(i));
                    }
                }
            } else {
                chk = chk + getGdict().get(cki.charAt(i)) + getCdict().get(lk);
            }
            lk -= 1;
        }
        return chk;
    ...
    public static String cwchange(String data) {
        if (data.indexOf(".") < 0) {
            data = data + ".0";
        }
        ...
        for (int i = 0; i < ikl; i++) {
            // 有可能一个字符串全是0的情况
            if ("".equals(cschange(cski.get(i)))) {
                // 此时不需要将数字标识符引入
                chk = chk + cschange(cski.get(i));
            } else {
...
//        }
    ...
    private static Map<Integer, String> getCdict() {
        cdict = new HashMap<>();
        cdict.put(1, "");
        cdict.put(2, "十");
        cdict.put(3, "百");
    ...
    private static Map<Character, String> getGdict() {
        gdict = new HashMap<>();
        gdict.put('0', "零");
        gdict.put('1', "一");
        gdict.put('2', "二");
